namespace LectureRoomMgt.Models.Scheduler
{
    ...
    public class SchedulerTaskService : ISchedulerEventService<TaskViewModel>
    {
        ...
        public SchedulerTaskService(IHttpContextAccessor httpContextAccessor, UserManager<ApplicationUser> userManager,WisdomAppDBContext wisdomAppDBContext)
        {
            _session = httpContextAccessor.HttpContext.Session; Context = wisdomAppDBContext;
        ...
        private IList<TaskViewModel> GetAllTasks()
        {
            using (Context)
            {
                ...
                {
                    result = Context.Tasks.ToList().Select(task => new TaskViewModel
                    {
                        TaskID = task.TaaskID,
                        Title = task.Title,
                        ...
                        //FacultyId =1, //task.Room.Floor.Block.FacultyId,
                        //BlockId =1, //task.Room.Floor.BlockId,
                        //FloorId =1 //task.Room.FloorId
                    }).ToList();
